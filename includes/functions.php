...
function sanitize_var($value) {
    if (!is_numeric($value)) {
        ...
        $value = preg_replace('amp', 'and', $value);
        $value = preg_replace('quot', '', $value);
        $value = preg_replace('039', '', $value);
...
function terms_box($terms_type, $selected_value) {
    ...
    if ($terms['enabled']) {
        ...
        $display_output .= '	<tr> ' .
                ...
                '		<td colspan="' . ($colspan - 1) . '"><textarea name="terms_content" cols="50" rows="8" readonly class="smallfont" style="width: 100%; height: 200px;" />' . preg_replace('<br>', "\n", $terms['content']) . '</textarea></td> ' .
                '	</tr> ' .
                '	<tr class="reguser"> ' .
                '		<td align="right" class="contentfont">&nbsp;</td> ' .
